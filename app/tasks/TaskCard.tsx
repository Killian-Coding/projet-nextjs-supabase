...
export default function TaskCard({ task }: TaskCardProps) {
  ...
  const statusConfig = {
    todo: { label: 'À faire', color: 'bg-yellow-100 text-yellow-800 border-yellow-200', emoji: '📝' },
    in_progress: { label: 'En cours', color: 'bg-blue-100 text-blue-800 border-blue-200', emoji: '⚡' },
    done: { label: 'Terminé', color: 'bg-green-100 text-green-800 border-green-200', emoji: '✅' }
  }
  ...
  return (
    <Card className={`p-6 ${isDeleting ? 'opacity-50' : ''}`}>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <Button variant="danger" size="sm" onClick={handleDelete} disabled={isUpdating || isDeleting}>🗑️ Supprimer</Button>
      </div>
