  ...
  return (
    <Card className={`p-6 ${isDeleting ? 'opacity-50' : ''}`}>
      <div className="flex justify-between items-start mb-3">
        ...
        <span className={`px-3 py-1 rounded-full text-xs font-medium border ${currentStatus.color}`}>
          {currentStatus.label}
        </span>
      </div>
      {task.description && <p className="text-gray-600 mb-4">{task.description}</p>}
      <div className="flex flex-wrap gap-2">
        {task.status !== 'in_progress' && <Button variant="secondary" size="sm" onClick={() => handleStatusChange('in_progress')} disabled={isUpdating || isDeleting}>Démarrer</Button>}
